...
- [x] **Viewer 改名反向同步 Kimi CLI。** 根因是 Kimi `/sessions` 读取 private query-store 索引，直接写 `state.json` 不会可靠更新该索引。改名现通过 Kimi Code 临时本地回环 API 的官方 `POST /api/v1/sessions/{id}/profile` 路径执行，由 Kimi 同步写入状态和索引；目标会话 `session_57e7460b-4876-4a76-a417-6b7bf386a138` 已实机验证 viewer 改名后 CLI `/sessions` 显示新标题。
- [x] **Kimi hooks 更新内嵌终端标签运行状态。** Kimi hook 的实际 payload 使用 `sessionId` / `promptId` camelCase，而 Rust `TerminalTurnPayload` 原先未声明对应 serde rename，导致 session ID 被反序列化为 `None`；无 transcript path 的 completed signal 因而无法命中 Kimi terminal tab，三点执行状态不会清除。现已兼容 camelCase 与 snake_case，Kimi 在 path 尚未可解析时以 session ID 匹配终端 tab，并保留 main wire 路径有限重试供桌面宠物任务使用；实机确认消息结束后状态恢复正常。
